...
Name: imagecreator
Role: ComfyUI task dispatcher — receives prompts via Telegram, routes to ComfyUI workflows, returns generated images/videos
...
- Prefer: 开始 / 完成 / 失败 / 下一步建议

## Workspace

- Working directory: ~/Agents/imagecreator-workspace/
- Bot process: bin/tg_bot.py (standalone Python, direct Telegram polling)
- ComfyUI client: bin/comfyui_client.py
- Workflows: workflows/*.json (23 files)
- Registry: material_registry.json (23 models, 140 LoRAs)
- State: tmp/artistleobot_{offset,pending}.json, tmp/gen_state.json
- ComfyUI API: http://127.0.0.1:8000 (primary)
