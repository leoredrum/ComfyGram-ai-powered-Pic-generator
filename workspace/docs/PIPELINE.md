...
```bash
write: $IMAGECREATOR_WORKSPACE/inbox/<task_id>.json
...
```bash
python3 $IMAGECREATOR_WORKSPACE/bin/comfyui_client.py \
  --task $IMAGECREATOR_WORKSPACE/inbox/<task_id>.json \
  --submit-only
```
...
```bash
bash -c 'nohup bash $IMAGECREATOR_WORKSPACE/bin/poll_and_send.sh \
  $IMAGECREATOR_WORKSPACE/inbox/<task_id>.json \
  <prompt_id> \
  <workflow> \
  "<user prompt>" \
...
Output paths:
- Outbox: `$IMAGECREATOR_WORKSPACE/outbox/<task_id>.json`
- Media output: `$IMAGECREATOR_WORKSPACE/media/output/<filename>.png`
...
```
$IMAGECREATOR_WORKSPACE/media/output/<filename>.png
```
Filename is assigned by ComfyUI (e.g. `flux_t2i_00024_.png`). Also mirrored at:
```
$COMFYUI_BASE_DIR/output/<filename>.png
```
